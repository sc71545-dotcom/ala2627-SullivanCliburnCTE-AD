...
function moveTarget() {
  target.style.left = `${58 + Math.random() * 30}%`;
  target.style.top = `${22 + Math.random() * 55}%`;
}
...
function shoot() {
  ...
  setTimeout(() => {
    ...
    if (hit) {
      game.hits += 1;
      hitsText.textContent = game.hits;
      target.classList.add('hit');
      statusText.textContent = 'Direct hit! New target incoming';
      setTimeout(() => { target.classList.remove('hit'); target.classList.remove('destroyed'); moveTarget(); }, 500);
    } else {
      statusText.textContent = 'Missed. Adjust your position.';
    }
    game.locked = false;
  }, 500);
}
...
function resetGame() {
  ...
  target.classList.remove('hit', 'destroyed'); renderTank(); moveTarget();
}

document.querySelectorAll('[data-move]').forEach(button => button.addEventListener('click', () => moveTank(button.dataset.move)));
document.addEventListener('keydown', event => {
  const keys = { ArrowUp: 'up', w: 'up', s: 'down', ArrowDown: 'down', a: 'left', ArrowLeft: 'left', d: 'right', ArrowRight: 'right' };
